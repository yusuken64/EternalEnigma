...
namespace TWC.Actions
{
	...
	public class Paint : TWCBlueprintAction, ITWCAction
	{
		...
		public override void DrawSceneGUI(Rect _sceneView)
		{
			...
			if (registeredToSceneView)
			{
				...
				using (new GUILayout.VerticalScope("Box", GUILayout.Width(300)))
				{
					...
					GUILayout.Label("Paint: LEFT CTRL + Left or Right Mouse button");
				}
				...
				if (layerIndex < tileWorldCreator.twcAsset.mapBlueprintLayers.Count)
				{
					using (new GUILayout.VerticalScope("Box"))
					{
						...
						{
							GUI.color = Color.white;
							GUILayout.Label("Brush Size: " + (paintBrushSize + 1));
							paintBrushSize = (int)GUILayout.HorizontalSlider(paintBrushSize, 0, 20);
						}
					}
		...
		void SetCell(Vector3 _gridPos, bool _state)
		{
			...
			if (tileWorldCreator.twcAsset.mapOrientation == TileWorldCreatorAsset.MapOrientation.XZ)
			{
				for (int i = -paintBrushSize; i < paintBrushSize+1; i ++)
				{
					for (int j = -paintBrushSize; j < paintBrushSize+1; j ++)
					{
						if ((int)_gridPos.x + i >= 0 && (int)_gridPos.x + i < paintMap.GetLength(0) && (int)_gridPos.z + j >= 0 && (int)_gridPos.z + j < paintMap.GetLength(1))
						{
							//Debug.Log("Paint");
							paintMap[(int)_gridPos.x + i, (int)_gridPos.z + j] = _state; 
						}
					}
				}
			}
