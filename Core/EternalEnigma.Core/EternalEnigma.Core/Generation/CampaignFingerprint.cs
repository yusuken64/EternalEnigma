...
public static class CampaignFingerprint
{
    ...
    public static string CanonicalText(Campaign campaign)
    {
        ...
        foreach (var r in campaign.Routes.OrderBy(r => r.Id, StringComparer.Ordinal)) Row("route", r.Id, r.From, r.To, r.Requirement, r.Form, r.Required, r.IsProgressionBoundary, r.ShortcutKind, r.UnlockingEndpoint, r.KeyId, r.KeyLocationId, r.IsWarp);
