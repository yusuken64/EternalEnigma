...
try {
    ...
    if (name === 'harness') {
        ...
        if (!['EditMode', 'PlayMode', 'Skills', 'Demo', 'Town'].includes(mode)) throw Error('Expected EditMode, PlayMode, Skills, Demo or Town.');
        const resultPath = join(root, `Temp/HarnessResults/${['Skills', 'Demo', 'Town'].includes(mode) ? 'PlayMode' : mode}.json`);
        const readSummary = () => {
