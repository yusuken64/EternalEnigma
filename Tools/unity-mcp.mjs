...
try {
    ...
    if (name === 'harness') {
        ...
        if (!['EditMode', 'PlayMode', 'Skills', 'Demo', 'Town', 'Overworld', 'Campaign', 'Autoplay', 'Enemies', 'Heroes', 'Classes', 'AllyAI'].includes(mode)) throw Error('Expected EditMode, PlayMode, Skills, Demo, Town, Overworld, Campaign, Autoplay, Enemies, Heroes, Classes or AllyAI.');
        const resultPath = join(root, `Temp/HarnessResults/${['Skills', 'Demo', 'Town', 'Overworld', 'Campaign', 'Autoplay', 'Enemies', 'Heroes', 'Classes', 'AllyAI'].includes(mode) ? 'PlayMode' : mode}.json`);
        const readSummary = () => {
