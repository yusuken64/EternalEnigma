...
try {
    ...
    if (name === 'harness') {
        ...
        if (!['EditMode', 'PlayMode', 'Skills', 'Demo', 'Town', 'Overworld'].includes(mode)) throw Error('Expected EditMode, PlayMode, Skills, Demo, Town or Overworld.');
        const resultPath = join(root, `Temp/HarnessResults/${['Skills', 'Demo', 'Town', 'Overworld'].includes(mode) ? 'PlayMode' : mode}.json`);
        const readSummary = () => {
