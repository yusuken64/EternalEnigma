...
try {
    ...
    if (name === 'harness') {
        ...
        if (!['EditMode', 'PlayMode', 'Skills', 'Demo', 'Town', 'Overworld', 'Campaign'].includes(mode)) throw Error('Expected EditMode, PlayMode, Skills, Demo, Town, Overworld or Campaign.');
        const resultPath = join(root, `Temp/HarnessResults/${['Skills', 'Demo', 'Town', 'Overworld', 'Campaign'].includes(mode) ? 'PlayMode' : mode}.json`);
        const readSummary = () => {
